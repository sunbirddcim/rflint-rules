...
def project_meta(path):
    ...
    for rfile in all_robot_files(project_file(PureWindowsPath(path).parent)):
        ...
        for keyword in rf.walk(Keyword):
            rfmeta.defs.append({'name': keyword.name, 'line': keyword.linenumber, 'src': keyword})
            for row in keyword.rows:
                rfmeta.uses.extend(extract_used_keywords(row.cells))
            rfmeta.uses = list(set(rfmeta.uses))
        for table in rf.tables:
            if isinstance(table, SettingTable):
                for statement in table.statements:
                    if statement[0].lower() in ['test setup', 'test teardown', 'suite setup', 'suite teardown',
                                                'test template']:
                        rfmeta.uses.extend(extract_used_keywords(statement[1:]))
            elif isinstance(table, TestcaseTable):
                for testcase in table.testcases:
                    for statement in testcase.statements:
                        rfmeta.uses.extend(extract_used_keywords(statement))
        ret.append(rfmeta)
    return ret
